...
ll MOD=1e9+7; 
ll fact[N];
ll bigmod(ll a,ll b,ll m){if(b==0) return 1;ll x=bigmod(a,b/2,m);x=(x*x)%m;if(b%2)x=(x*a)%m;return x;}

ll ncr(ll n, ll r)
{
	if (r > n) return 0;
	ll ans = fact[n] * bigmod(fact[r], MOD - 2, MOD);
	ans %= MOD;
	ans *= bigmod(fact[n - r], MOD - 2, MOD);
	ans %= MOD;
	return ans;
}
void f()
